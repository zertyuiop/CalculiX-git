...
begin
    if (threads_num < max_threads_num) && (dirstmp.size > 0)
        threads << Thread.new do
	        ...
	        system(solver)
		    Dir.chdir("../lua-femtk/")
		    postprocessor = "lua frd2exo.lua -f -2 -sets " + dr + "/sets.nc -voln 100 -surfn 200 " + dr + "/" + work_file + " " + dr + "/" + project_name + ".exo 2>&1 | tee -a " + project_name + ".co.dump"
		    system(postprocessor)
		    Dir.chdir("../")
			Dir.chdir(dr)
			compress = "tar cvf - . | xz -9 -c -e -v -T 0 - > " + File.basename(Dir.getwd) + ".txz"
			system(compress)
			#host = `hostname`
